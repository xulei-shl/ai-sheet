...
> AI-Sheet 综合设计与架构文档（v3，2026-06-07）
>
...
| LLM | pi-ai providers | 9 类 API（Anthropic/OpenAI/DeepSeek/Google…） |
...
│   │                               pi-ai / @sinclair/typebox）
│   ├── tsconfig.json
│   └── src/
│       ├── main.ts                （stdin JSONL 路由 + 启动 AgentSession）
│       ├── agent.ts               （createSheetAgent：注册自定义工具 + 系统提示词）
...
`types/agent.ts`：
- `AgentMessage { id, role, content, isStreaming? }`
- `AgentContext { currentTab, loadedFiles, selectedColumns, sampleDataPreview? }`
- `AgentStatus`
...
const model = defaultModel
  ? getModel(defaultModel.providerType as any, defaultModel.modelId as any)
  : undefined;
...
└──────────┬─────────────┘    └──────────┬──────────────┘
  ...
  invoke('send_agent_message', JSON.stringify({type:'batch_start',...}))
   ...
   Rust: sidecar.send_batch_command('batch_start', {params})
                          ▼
   Node.js: handleBatchStart → new BatchRunner().run(...)
                          ▼
