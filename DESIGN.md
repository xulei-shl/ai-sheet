...
> AI-Sheet 综合设计与架构文档（v5，2026-06-09）
>
> v5 变更：移除了独立 Direct LLM 流（`direct-llm.ts`），快捷按钮改为走常规 Agent 消息路径。
> 原 Direct LLM 模式与 `direct_llm_message` 协议已删除，BatchRunner 的独立 `stream()` 调用不受影响。
>
...
│   ├── tsconfig.json
│   └── src/
│       ├── main.ts                （stdin JSONL 路由 + 启动 AgentSession + 双 Dispatcher + .env 加载）
│       ├── agent.ts               （createSheetAgent：注册自定义工具 + 系统提示词 + provider-map 拆分）
