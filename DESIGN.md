...
> 实时计算，兼容不同模型的上下文窗口；ConfigPage 新增"上下文窗口"输入框，用户
> 可手动填写模型的实际值（如 128000），留空默认 128K；`models` 表新增
> `context_window` 列（DB migration v8）；移除 `App.tsx` 旧全局状态条。
>
...
> 参数传递会话目录；每次启动创建新 JSONL 文件，重启不自动加载；会话文件保留磁盘供后续
> 历史浏览功能使用。关闭进程不再丢失对话。
>
...
- **AgentFooter**（`src/components/agent/AgentFooter.tsx`）：对话面板底部状态栏，
  ...
  - 📊 上下文使用率（`inputTokens / contextWindow`），颜色梯度：<60% 绿 /
    60-85% 黄 / >85% 红；`contextWindow` 来自模型配置页用户设定值，
    未设置时默认 128K
...
- `set_session_dir(dir)`：在启动前由 `lib.rs` 注入会话存储目录
  （`app_data_dir/sessions/`），通过 `--session-dir` 参数传给 Node.js，
  Agent 的 `SessionManager.create()` 将 JSONL 会话文件写入该目录。
...
│   │   ├── sidecar_manager.rs （Node.js 进程管理 + 心跳 + 超时 + --db-dir + --session-dir + send_set_cwd）
...
│   ├── tsconfig.json
│   └── src/
│       ├── main.ts                （stdin JSONL 路由 + 启动 AgentSession + 双 Dispatcher + .env 加载 + --db-dir/--session-dir 解析 + set_cwd 处理）
│       ├── agent.ts               （createSheetAgent：注册自定义工具 + provider-map 拆分 + ResourceLoader 自动发现技能 + agentsFilesOverride 注入 AGENTS.md + SessionManager 持久化切换）
...
`SessionManager` 由 pi-agent SDK 提供，支持两种模式：
...
**当前实现**：`src-agent/src/agent.ts:103` 根据是否传入 `--session-dir` 参数
自动选择：有路径时用 `SessionManager.create(initialCwd, sessionDir)`，否则退回
`SessionManager.inMemory()`。

**存储路径**：`app_data_dir/sessions/<session-id>.jsonl`（Windows 典型路径
`C:\Users\<user>\AppData\Roaming\com.ai-sheet.desktop\sessions\`）。

**启动行为**：每次 `npm run tauri:dev` 或生产启动，sidecar 重新初始化时，
`createSheetAgent` 调用 `SessionManager.create()` → 生成新的 UUID 会话文件。
**重启不自动加载历史会话**，始终从新文件开始。历史会话 JSONL 保留磁盘供后续
历史浏览功能使用。

**重置行为**：前端"清空对话"按钮触发 `session.sessionManager.newSession()`，
在当前文件内创建新分支，不会丢失已写入的会话条目。

**修改涉及 4 处代码**：

| 文件 | 改动 |
|------|------|
| `src-agent/src/agent.ts:16,103` | 函数签名加 `sessionDir?` 参数，条件选择 `SessionManager.create()` |
| `src-agent/src/main.ts:29-37,118` | 解析 `--session-dir` CLI 参数并传入 |
| `src-tauri/src/services/sidecar_manager.rs:31,47-51,70-73` | 新增 `session_dir` 字段 + setter + spawn 时传参 |
| `src-tauri/src/lib.rs:150-158` | 在 `setup` 阶段创建 `app_data_dir/sessions/` 目录并注入 sidecar |

**JSONL 格式特性**（pi-agent SDK 内置）：
- 追加写：每条消息/操作作为一行 JSON 记录，支持 `id`/`parentId` 链式追踪
- 树状分支：`branch()` 回溯到历史节点、`forkFrom()` 复制会话文件
- 元数据：`appendSessionInfo()` 设置会话名称，`appendLabelChange()` 标记关键节点
- 上下文压缩：Compaction 摘要作为特殊 entry 记录在 JSONL 中，不影响树结构
- API 详见 `docs/pi-docs-playbook/.../session-format.md`
...
- **上下文占比**由前端计算：`inputTokens / contextWindow`。`contextWindow` 来源：
  ① 模型配置页用户填写的值（持久化到 `models.context_window` 列）
  ② 未填写时默认 `128_000`（`provider-map.ts: buildModel` 硬编码）
  不依赖任何查找表或 provider 默认值映射。
- `contextWindow` 为 0 或 `inputTokens` 为 0 时隐藏 📊 段。
