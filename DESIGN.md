...
> AI-Sheet 综合设计与架构文档（v6，2026-06-09）
>
> v6 变更：pi agent 动态工作目录机制——默认 cwd 为 DB 数据目录，加载 Excel 后自动切换到文件所在目录；
> 通过 DefaultResourceLoader + skillsOverride 注入 python-processing skill，与 cwd 解耦；
...
│       ├── agent.ts               （createSheetAgent：注册自定义工具 + 系统提示词 + provider-map 拆分 + ResourceLoader + skillsOverride）
...
**Skill 持久化**：python-processing skill 通过 `DefaultResourceLoader` 的 `skillsOverride` 回调
程序化注入，不依赖 cwd 目录扫描。无论工作目录如何变化，该 skill 始终出现在 agent 可用列表中。
运行时行为与目录扫描发现一致：启动时仅注入 name + description 元数据到系统提示词，
...
const pythonSkill: Skill = {
  ...
  sourceInfo: createSyntheticSourceInfo(filePath, { source: 'custom', scope: 'project' }),
...
const loader = new DefaultResourceLoader({
  ...
  skillsOverride: (current) => ({
    skills: current.skills.some((s) => s.name === 'python-processing')
      ? current.skills : [...current.skills, pythonSkill],
    diagnostics: current.diagnostics,
  }),
});
```

**为什么不重建 session**：`createAgentSessionRuntime()` 会丢失对话历史、
需重新订阅事件和绑定扩展，代价过大。steer 通知 + 逻辑 cwd 追踪是更轻量的方案。

公式应用 / 写结果完成后，前端订阅 `agent-event` 中 `agent_tool_end`，
若工具为 `write_excel` / `apply_formula` 可选择刷新当前预览。
